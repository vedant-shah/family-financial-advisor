...
def test_new_people_added_existing_not_duplicated(root):
    _household(root, "| vedant | vedant | self | yes |\n| rakesh | rakesh | Father | yes |\n")
    _obs(
        root,
        "- 2026-06-14 — (via vedant, about Rakesh (dad)): father, also earning <!-- id:a -->\n"
    ...
    assert hh.count("| rakesh |") == 1   # existing not duplicated
...
def test_contradiction_is_staged_not_overwritten(root):
    # rakesh is on record as earning; a new observation says he retired.
    _household(root, "| vedant | vedant | self | yes |\n| rakesh | rakesh | Father | yes |\n")
    _obs(root, "- 2026-06-16 — (via vedant, about Rakesh (dad)): dad has retired now <!-- id:d -->\n")

    promote_observations(root, today=TODAY)

    hh = (root / "family" / "household.md").read_text()
    assert "| rakesh | rakesh | Father | yes |" in hh  # NOT overwritten
    disc = (root / "working" / "discrepancies.md").read_text()
    assert "rakesh" in disc
